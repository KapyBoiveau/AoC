...
for r in open('./2016/day20/input.txt', 'r').readlines():
    ...
    merges = set()
    for rng in ips:
        u = union(s_e, rng) # try to union the new interval with every one we already have
        if len(u) == 1:
            merges.add(u[0]) # add to merges when union, don't add to ranges because we may have to union the merges
        else:
            ranges.add(rng) # add to ranges everything that doesn't merge with the new interval
